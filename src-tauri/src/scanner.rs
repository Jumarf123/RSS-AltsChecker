...
const MAX_STEAM_VDF_FILE_BYTES: u64 = 8 * 1024 * 1024;
...
const AUDIT_REGISTRY_PATH: &str = r"Software\RSS-AltsChecker";
...
static MINECRAFT_LOG_PATTERNS: Lazy<Vec<Regex>> = Lazy::new(|| {
    vec![
        ...
        Regex::new(r"(?i)\bauth_player_name[=:]\s*([A-Za-z0-9_]{3,16})")
            .expect("regex must compile"),
        Regex::new(
            r"(?i)\bselected\s+(?:minecraft\s+)?(?:account|profile)[:=]\s*([A-Za-z0-9_]{3,16})",
        )
        .expect("regex must compile"),
        Regex::new(r"(?i)\b(?:username|displayname|profileName)[=:]\s*([A-Za-z0-9_]{3,16})")
            .expect("regex must compile"),
    ]
});
...
#[derive(Debug, Clone, Serialize, Default)]
pub struct SteamBanStatus {
    pub vac_banned: Option<bool>,
    pub number_of_vac_bans: Option<u32>,
    pub number_of_game_bans: Option<u32>,
    pub days_since_last_ban: Option<u32>,
    pub community_banned: Option<bool>,
    pub notes: Vec<String>,
}

#[derive(Debug, Clone, Serialize)]
pub struct ThirdPartyCheck {
    pub service: String,
    pub status: String,
    pub label: String,
    pub detail: String,
...
pub struct SteamAlt {
    ...
    pub bans: SteamBanStatus,
    pub third_party: Vec<ThirdPartyCheck>,
    pub confidence: ConfidenceLevel,
    pub sources: Vec<String>,
}

#[derive(Debug, Clone, Serialize, Default)]
pub struct SteamCheckReport {
    pub generated_at: String,
    pub steam_accounts: Vec<SteamAlt>,
...
#[derive(Debug, Clone, Serialize, Default)]
pub struct SystemHwid {
    pub primary_hwid: String,
    pub raw: String,
    pub md5: String,
    pub sha256: String,
...
pub struct AuditReport {
    ...
    pub minecraft_file_path: Option<String>,
...
struct PersistedAccountFile {
    version: u32,
    updated_at: String,
    records: Vec<PersistedAccountRecord>,
}
...
struct AuditManifest {
    version: u32,
    updated_at: String,
    registry_had_discord: bool,
    registry_had_steam: bool,
    minecraft_had_records: bool,
}

#[derive(Debug, Clone, Default)]
struct AuditSnapshot {
    discord: Option<Vec<PersistedAccountRecord>>,
    steam: Option<Vec<PersistedAccountRecord>>,
    minecraft: Option<Vec<PersistedAccountRecord>>,
}
...
pub fn default_steam_report_path() -> PathBuf {
    env::current_dir()
        .unwrap_or_else(|_| PathBuf::from("."))
        .join("RSS-AltsChecker-steam-results.json")
}
...
pub fn scan(options: &ScanOptions) -> Result<ScanReport, String> {
    ...
    let audit = persist_audit_snapshot(AuditSnapshot {
        discord: Some(
            discord_scan
                .accounts
                .iter()
                .filter(|account| account.confidence == ConfidenceLevel::Max)
                .map(persisted_record_from_discord)
                .collect(),
        ),
        steam: None,
        minecraft: Some(
            minecraft_scan
                ...
                .iter()
                .filter(|account| account.confidence == ConfidenceLevel::Max)
                .map(persisted_record_from_minecraft)
                .collect(),
        ),
    });
    forensic_signals.extend(audit.signals.iter().cloned());
...
fn scan_minecraft_accounts(
    ...
    for entry in map.into_values() {
        let (kept, confidence, reason, account_source_count) = classify_minecraft_candidate(&entry);
        let sources = entry.sources.into_iter().collect::<Vec<_>>();

        debug.push(MinecraftDetectionDebug {
            username: entry.username.clone(),
            kept,
            confidence: kept.then_some(confidence),
            reason,
...
fn classify_minecraft_candidate(
    entry: &MinecraftCandidate,
) -> (bool, ConfidenceLevel, String, usize) {
...
fn scan_discord_accounts(
    ...
    for entry in map.into_values() {
        ...
        let confidence = classify_discord_candidate(&entry);
...
fn scan_single_discord_leveldb_dir(
                ...
                match file.content {
                    ParsedFileContent::Ldb(ldb) => {
                        for entry in ldb.entries {
                            ...
                            extract_discord_accounts_from_bytes(
                                &entry.user_key,
                                &source,
                                map,
                                DiscordEvidenceKind::OwnerStore,
                            );
                            extract_discord_accounts_from_bytes(
                                &entry.value,
                                &source,
                    ...
                    ParsedFileContent::TextLog(log) => {
                        ...
                        extract_discord_accounts_from_text(
                            &text,
                            &source,
                            map,
                            DiscordEvidenceKind::Generic,
                        );
                    }
    ...
    for entry in entries.flatten() {
        ...
        extract_discord_accounts_from_bytes(&bytes, &source, map, DiscordEvidenceKind::Generic);
    }
...
fn extract_discord_accounts_from_bytes(
    ...
    source: &str,
    map: &mut HashMap<String, DiscordCandidate>,
    evidence_kind: DiscordEvidenceKind,
) {
    if bytes.is_empty() {
        return;
    }
    ...
    if let Ok(text) = std::str::from_utf8(bytes) {
        extract_discord_accounts_from_text(text, source, map, evidence_kind);
    } else {
        let lossy = String::from_utf8_lossy(bytes);
        extract_discord_accounts_from_text(&lossy, source, map, evidence_kind);
    }

    for chunk in extract_printable_chunks(bytes, 80, 180) {
        extract_discord_accounts_from_text(&chunk, source, map, evidence_kind);
    }
}
...
fn extract_discord_accounts_from_context_text(
    ...
    for captures in DISCORD_USER_PROFILE_URL_RE.captures_iter(text) {
        if let Some(id) = captures.get(1).map(|value| value.as_str()) {
            insert_discord_candidate(map, None, Some(id), source, DiscordEvidenceKind::Context);
        }
    }
    ...
    for captures in DISCORD_ENCODED_USER_RE_A.captures_iter(text) {
        if let (Some(username), Some(id)) = (
            ...
            insert_discord_candidate(
                map,
                Some(username),
                Some(id),
                source,
                DiscordEvidenceKind::Context,
            );
        }
    }
...
fn extract_discord_accounts_from_text(
    ...
    extract_discord_accounts_from_text_variant(text, source, map, evidence_kind);

    let normalized = normalize_escaped_json_text(text);
    if normalized != text {
        extract_discord_accounts_from_text_variant(&normalized, source, map, evidence_kind);
    }
}
...
fn extract_discord_accounts_from_text_variant(
    ...
    if trimmed.starts_with('{') || trimmed.starts_with('[') {
        if let Ok(value) = serde_json::from_str::<Value>(trimmed) {
            collect_discord_accounts_from_json(&value, source, map, 0, false, evidence_kind);
        }
    }
    ...
    for captures in DISCORD_USER_ID_RE_A.captures_iter(text) {
        ...
        if let (Some(username), Some(id)) = (
            ...
            insert_discord_candidate(map, Some(username), Some(id), source, evidence_kind);
        }
    }
...
fn classify_discord_candidate(entry: &DiscordCandidate) -> ConfidenceLevel {
    if entry.owner_store_hits > 0 && entry.named_hits > 0 {
        return ConfidenceLevel::Max;
    }

    if entry.owner_store_hits >= 2 || (entry.token_hits > 0 && entry.sources.len() >= 2) {
        return ConfidenceLevel::Max;
    }

    if entry.named_hits > 0 && entry.evidence_hits >= 2 && entry.sources.len() >= 2 {
        return ConfidenceLevel::Max;
    }
...
fn insert_discord_candidate(
    ...
    match evidence_kind {
        DiscordEvidenceKind::OwnerStore => entry.owner_store_hits += 1,
        DiscordEvidenceKind::Context => entry.context_hits += 1,
        DiscordEvidenceKind::Token => entry.token_hits += 1,
        DiscordEvidenceKind::Generic => {}
    }
...
fn is_discord_owner_store_key(user_key: &[u8]) -> bool {
    ...
    key_text.contains("multiaccountstore")
        ...
        || (key_text.contains("discord") && key_text.contains("token"))
        || key_text.contains("current_user")
        || key_text.contains("usersettingsstore")
}
...
pub fn run_steam_check(options: &ScanOptions) -> Result<SteamCheckReport, String> {
    check_cancelled(options)?;

    let mut warnings = Vec::new();
    let profiles = collect_user_profiles(&mut warnings);
    let steam_roots = collect_steam_roots(&profiles, &mut warnings);
    let mut scanned_locations = steam_roots
        .iter()
        .map(|path| format!("steam_root: {}", path.display()))
        .collect::<Vec<_>>();

    let mut accounts = scan_steam_accounts(&steam_roots, &mut warnings, options)?;
    check_cancelled(options)?;
    enrich_steam_accounts(&mut accounts, &mut warnings);
    check_cancelled(options)?;
    ...
    let audit = persist_audit_snapshot(AuditSnapshot {
        ...
        steam: Some(
            accounts
                .iter()
                .filter(|account| account.confidence == ConfidenceLevel::Max)
                .map(persisted_record_from_steam)
                .collect(),
        ),
        minecraft: None,
    });

    let mut forensic_signals = Vec::new();
    if accounts.is_empty() {
        forensic_signals.push("Steam: аккаунты не обнаружены".to_string());
    }
    forensic_signals.extend(audit.signals.iter().cloned());
    ...
    let output_path = default_steam_report_path();
    if let Some(parent) = output_path.parent() {
        if let Err(error) = fs::create_dir_all(parent) {
            warnings.push(format!(
                "Failed to create Steam report directory {}: {error}",
                parent.display()
            ));
        }
    }
    if let Ok(json) = serde_json::to_vec_pretty(&SteamCheckReport {
        generated_at: Local::now().to_rfc3339(),
        steam_accounts: accounts.clone(),
        forensic_signals: forensic_signals.clone(),
        scanned_locations: scanned_locations.clone(),
        warnings: warnings.clone(),
        audit: audit.clone(),
    }) {
        if let Err(error) = fs::write(&output_path, json) {
            warnings.push(format!(
                "Failed to write Steam report {}: {error}",
                output_path.display()
            ));
        }
    }
...
pub fn collect_system_hwid() -> SystemHwid {
    let mut warnings = Vec::new();
    let motherboard_uuid = read_first_command_value(
        "Motherboard UUID",
        &[("wmic", &["csproduct", "get", "uuid", "/value"] as &[&str])],
        &mut warnings,
    );
    let bios_serial = read_first_command_value(
        "BIOS serial",
        &[(
            "wmic",
            &["bios", "get", "serialnumber", "/value"] as &[&str],
        )],
        &mut warnings,
    );
    let disk_serial = read_first_command_value(
        "System disk serial",
        &[(
            "wmic",
            &["diskdrive", "get", "serialnumber", "/value"] as &[&str],
        )],
        &mut warnings,
    );
    let machine_guid = read_machine_guid(&mut warnings);
    let mac_address = read_first_command_value(
        "MAC address",
        &[(
            "wmic",
            &["nicconfig", "get", "MACAddress", "/value"] as &[&str],
        )],
        &mut warnings,
    );
    ...
    let user_name = env::var("USERNAME").ok().filter(|value| !value.is_empty());
    let mb = normalize_hwid_part(motherboard_uuid.as_deref());
    let bios = normalize_hwid_part(bios_serial.as_deref());
    let disk = normalize_hwid_part(disk_serial.as_deref());
    let guid = normalize_hwid_part(machine_guid.as_deref());
    let raw = format!("v1|MB={mb}|BIOS={bios}|DISK={disk}|GUID={guid}");
    let md5 = format!("{:x}", md5::compute(raw.as_bytes())).to_uppercase();
    let sha256 = format!("{:X}", Sha256::digest(raw.as_bytes()));
    let primary_hwid = group_hwid(&sha256, 4, 4);
...
fn read_first_command_value(
    ...
    for (program, args) in commands {
        let output =
            match run_hidden_command(program, args).and_then(|mut command| command.output()) {
                Ok(output) => output,
                Err(error) => {
                    warnings.push(format!("{label}: {program} unavailable: {error}"));
                    continue;
                }
            };
        if !output.status.success() {
            ...
            if !stderr.is_empty() {
                warnings.push(format!("{label}: {program} failed: {stderr}"));
            }
            continue;
        }

        let stdout = String::from_utf8_lossy(&output.stdout);
        if let Some(value) = parse_command_first_value(&stdout) {
            return Some(value);
        }
    }
    None
...
fn run_hidden_command(program: &str, args: &[&str]) -> std::io::Result<std::process::Command> {
...
fn parse_command_first_value(output: &str) -> Option<String> {
    output.lines().find_map(|line| {
        ...
        if value.is_empty()
            || matches!(
                lower.as_str(),
                "uuid" | "serialnumber" | "serial number" | "macaddress" | "mac address"
            )
        {
            return None;
        }
        ...
        let value = value
            ...
            .collect::<Vec<_>>()
            .join(" ");
        if is_placeholder_hwid_value(&value) {
            None
        } else {
            Some(value)
        }
...
fn is_placeholder_hwid_value(value: &str) -> bool {
...
fn normalize_hwid_part(value: Option<&str>) -> String {
    value
        .map(|value| {
            value
                .trim()
        ...
        .filter(|value| !is_placeholder_hwid_value(value))
        .map(|value| value.to_ascii_uppercase())
        .unwrap_or_else(|| "UNKNOWN".to_string())
}

fn group_hwid(value: &str, group_size: usize, groups: usize) -> String {
    value
        .chars()
...
pub fn startup_audit_check() -> AuditReport {
    inspect_audit_integrity()
}

fn collect_steam_roots(profiles: &[UserProfile], warnings: &mut Vec<String>) -> Vec<PathBuf> {
    let mut roots = Vec::<PathBuf>::new();
    let mut seen = HashSet::<String>::new();

    for path in registry_steam_paths(warnings) {
        push_unique_dir(&mut roots, &mut seen, path);
    }

    for var in ["ProgramFiles(x86)", "ProgramFiles"] {
        if let Some(base) = env::var_os(var).map(PathBuf::from) {
            push_unique_dir(&mut roots, &mut seen, base.join("Steam"));
        }
    }

    for profile in profiles {
        if let Some(local) = &profile.local {
            push_unique_dir(&mut roots, &mut seen, local.join("Steam"));
        }
    }
...
#[cfg(windows)]
fn registry_steam_paths(warnings: &mut Vec<String>) -> Vec<PathBuf> {
    ...
    for (root, subkey, value_name) in probes {
        ...
        match key.get_value::<String, _>(value_name) {
            ...
            Err(error) => warnings.push(format!(
                "Steam registry value unavailable: {subkey}\\{value_name}: {error}"
            )),
        }
    }
    paths
...
#[cfg(not(windows))]
fn registry_steam_paths(_warnings: &mut Vec<String>) -> Vec<PathBuf> {
    Vec::new()
}

fn scan_steam_accounts(
    roots: &[PathBuf],
    warnings: &mut Vec<String>,
    options: &ScanOptions,
) -> Result<Vec<SteamAlt>, String> {
    let mut map = HashMap::<String, SteamCandidate>::new();

    for root in roots {
        check_cancelled(options)?;
        let loginusers = root.join("config").join("loginusers.vdf");
        if loginusers.is_file() {
            scan_steam_loginusers(&loginusers, &mut map, warnings);
        }

        let userdata = root.join("userdata");
        if userdata.is_dir() {
            scan_steam_userdata(&userdata, &mut map, warnings);
        }
    }
    ...
    let mut out = map
        .into_values()
        .map(|entry| {
            let steam_id64 = entry.steam_id64;
            let third_party = build_third_party_checks(&steam_id64);
            let confidence = if entry.local_hits >= 2
                || entry.account_name.is_some()
                || entry.persona_name.is_some()
            {
                ConfidenceLevel::Max
            } else {
                ConfidenceLevel::Low
...
fn scan_steam_loginusers(
    path: &Path,
    map: &mut HashMap<String, SteamCandidate>,
    warnings: &mut Vec<String>,
) {
    let metadata = match fs::metadata(path) {
        ...
        Err(error) => {
            warnings.push(format!(
                "Failed to read Steam file {}: {error}",
                path.display()
            ));
            return;
        }
    };
    if metadata.len() == 0 || metadata.len() > MAX_STEAM_VDF_FILE_BYTES {
        return;
    }
    ...
    for user in parse_loginusers_vdf(&text) {
        insert_steam_candidate(
            map,
            &user.steam_id64,
            user.account_name.as_deref(),
            user.persona_name.as_deref(),
            None,
...
fn scan_steam_userdata(
    ...
    let entries = match fs::read_dir(userdata) {
        ...
        Err(error) => {
            warnings.push(format!(
                "Failed to read Steam userdata {}: {error}",
                userdata.display()
            ));
            return;
        }
    ...
    for entry in entries.flatten() {
        ...
        let Some(steam_id64) = normalize_steam_id(name) else {
            continue;
        };
...
#[derive(Default)]
struct ParsedSteamLoginUser {
    steam_id64: String,
    account_name: Option<String>,
    persona_name: Option<String>,
}

fn parse_loginusers_vdf(text: &str) -> Vec<ParsedSteamLoginUser> {
    let mut users = Vec::<ParsedSteamLoginUser>::new();
    let lines = text.lines().collect::<Vec<_>>();
    let mut index = 0usize;

    while index < lines.len() {
        let key = parse_vdf_quoted_key(lines[index]);
        if key
            .as_deref()
            .is_some_and(|value| normalize_steam_id(value).is_some())
        {
            let steam_id64 = normalize_steam_id(key.as_deref().unwrap()).unwrap();
            index += 1;
            ...
            while index < lines.len() {
                ...
                if let Some((field, value)) = parse_vdf_pair(line) {
                    if field.eq_ignore_ascii_case("AccountName") {
                        account_name = Some(value);
                    } else if field.eq_ignore_ascii_case("PersonaName") {
                        persona_name = Some(value);
                    }
                }
                index += 1;
            }
            users.push(ParsedSteamLoginUser {
                steam_id64,
                account_name,
                persona_name,
            });
...
fn parse_vdf_quoted_key(line: &str) -> Option<String> {
...
fn parse_vdf_pair(line: &str) -> Option<(String, String)> {
    ...
    let first_end = trimmed[1..].find('"')? + 1;
    let key = trimmed[1..first_end].to_string();
    let rest = trimmed[first_end + 1..].trim_start();
    if !rest.starts_with('"') {
        return None;
    }
    let value_end = rest[1..].find('"')? + 1;
    let value = rest[1..value_end].to_string();
    Some((key, value))
}
...
fn normalize_steam_id(value: &str) -> Option<String> {
    ...
    let parsed = trimmed.parse::<u64>().ok()?;
    if trimmed.len() >= 17 && parsed >= STEAM_ID64_BASE {
        return Some(parsed.to_string());
    }
    if parsed > 0 && parsed < STEAM_ID64_BASE {
        return Some(parsed.saturating_add(STEAM_ID64_BASE).to_string());
    }
    None
}

fn insert_steam_candidate(
    map: &mut HashMap<String, SteamCandidate>,
    steam_id64: &str,
    ...
    let Some(steam_id64) = normalize_steam_id(steam_id64) else {
        return;
    };

    let entry = map
        .entry(steam_id64.clone())
    ...
    if entry.account_name.is_none() {
        entry.account_name = account_name
            .map(str::trim)
            .filter(|value| !value.is_empty())
            .map(ToOwned::to_owned);
    }
    if entry.persona_name.is_none() {
...
pub fn build_third_party_checks(steam_id64: &str) -> Vec<ThirdPartyCheck> {
    let checked_at = Local::now().to_rfc3339();
    let bansearch_url = format!("https://bansearch.com/{steam_id64}");
    let csgobans_url = format!("https://www.csgobans.com/?steamid={steam_id64}");
    ...
    vec![
        third_party_check(
            ...
            "unknown",
            "Open",
            "SteamID копируется при открытии. Если сайт не заполнит поле автоматически, вставьте ID и нажмите Check.",
            &csgobans_url,
            &checked_at,
            false,
            false,
        ...
        third_party_check(
            ...
            "unknown",
            "Open",
            "Открывает готовую страницу BanSearch для этого SteamID.",
            &bansearch_url,
            &checked_at,
            false,
            false,
...
fn enrich_steam_accounts(accounts: &mut [SteamAlt], warnings: &mut Vec<String>) {
    if accounts.is_empty() {
        return;
    }

    enrich_steam_profiles_from_xml(accounts, warnings);
    enrich_steam_bans(accounts, warnings);
}

fn enrich_steam_profiles_from_xml(accounts: &mut [SteamAlt], warnings: &mut Vec<String>) {
    let agent = ureq::AgentBuilder::new()
        .timeout(std::time::Duration::from_secs(4))
        .build();

    for account in accounts.iter_mut() {
        if account.persona_name.is_some() && account.avatar_url.is_some() {
            continue;
        }
        ...
        let text = match response.into_string() {
            ...
            Err(error) => {
                warnings.push(format!(
                    "Steam profile parse failed for {}: {error}",
                    account.steam_id64
                ));
                continue;
            }
        ...
        if account.persona_name.is_none() {
            account.persona_name = extract_xml_tag(&text, "steamID");
        }
        if account.avatar_url.is_none() {
            account.avatar_url = extract_xml_tag(&text, "avatarFull")
                .or_else(|| extract_xml_tag(&text, "avatarMedium"));
        }
    }
}

fn enrich_steam_bans(accounts: &mut [SteamAlt], warnings: &mut Vec<String>) {
    ...
    let value = match agent.get(&url).call() {
        Ok(response) => match response.into_json::<Value>() {
            Ok(value) => value,
            Err(error) => {
                warnings.push(format!("Steam bans API response parse failed: {error}"));
                for account in accounts {
                    account
                        .bans
                        .notes
                        .push("Steam bans: ответ API не прочитан".to_string());
                }
                return;
            }
        },
        Err(error) => {
            warnings.push(format!("Steam bans API unavailable: {error}"));
            for account in accounts {
                account
                    .bans
                    .notes
                    .push("Steam bans: API недоступен".to_string());
            }
            return;
        }
    };
    ...
    for player in players {
        if let Some(id) = player.get("SteamId").and_then(Value::as_str) {
            by_id.insert(id.to_string(), player);
        }
    }

    for account in accounts {
        let Some(player) = by_id.get(&account.steam_id64) else {
            account
                .bans
                .notes
                .push("Steam bans: нет ответа по аккаунту".to_string());
            continue;
        };
        account.bans.vac_banned = player.get("VACBanned").and_then(Value::as_bool);
        account.bans.community_banned = player.get("CommunityBanned").and_then(Value::as_bool);
        account.bans.number_of_vac_bans = player
            .get("NumberOfVACBans")
            .and_then(Value::as_u64)
            .map(|v| v as u32);
        account.bans.number_of_game_bans = player
            .get("NumberOfGameBans")
            .and_then(Value::as_u64)
            .map(|v| v as u32);
        account.bans.days_since_last_ban = player
            .get("DaysSinceLastBan")
            .and_then(Value::as_u64)
            .map(|v| v as u32);
    }
}
...
fn steam_api_key() -> String {
    if let Ok(value) = env::var("STEAM_WEB_API_KEY") {
    ...
    const OBFUSCATED: [u8; 32] = [
        107, 104, 108, 108, 106, 24, 30, 106, 99, 110, 109, 110, 98, 111, 105, 105, 108, 98, 30,
        111, 24, 105, 104, 99, 107, 107, 99, 24, 99, 27, 99, 30,
    ];
    OBFUSCATED
        .iter()
        .map(|byte| (byte ^ XOR_KEY) as char)
        .collect()
}
...
fn extract_xml_tag(text: &str, tag: &str) -> Option<String> {
    ...
    let mut value = text[start..end].trim().to_string();
...
fn persisted_record_from_minecraft(account: &MinecraftAlt) -> PersistedAccountRecord {
    persisted_record(&account.username, &account.username, &account.sources)
}

fn persisted_record_from_discord(account: &DiscordAlt) -> PersistedAccountRecord {
    let id = account
        .id
        .clone()
        .unwrap_or_else(|| account.username.clone());
    persisted_record(&id, &account.username, &account.sources)
}

fn persisted_record_from_steam(account: &SteamAlt) -> PersistedAccountRecord {
    let label = account
        .persona_name
        .clone()
        .or_else(|| account.account_name.clone())
        .unwrap_or_else(|| account.steam_id64.clone());
    persisted_record(&account.steam_id64, &label, &account.sources)
}

fn persisted_record(id: &str, label: &str, sources: &[String]) -> PersistedAccountRecord {
    ...
    PersistedAccountRecord {
        ...
        source_kinds: sources
            .iter()
            .map(|source| classify_audit_source(source).to_string())
            .collect(),
    }
}

fn persist_audit_snapshot(snapshot: AuditSnapshot) -> AuditReport {
    let mut report = inspect_audit_integrity();
    let previous_discord = read_registry_account_file("DiscordAccounts");
    let previous_steam = read_registry_account_file("SteamAccounts");
    let previous_minecraft = read_minecraft_account_file();

    if let Some(current) = snapshot.discord.as_ref() {
        report.signals.extend(compare_previous_records(
            "Discord",
            &previous_discord.records,
            current,
        ));
        let merged = merge_persisted_records(previous_discord.records, current.clone());
        if let Err(error) = write_registry_account_file("DiscordAccounts", &merged) {
            report
                .signals
                .push(format!("Audit Discord registry write failed: {error}"));
        }
    }

    if let Some(current) = snapshot.steam.as_ref() {
        report.signals.extend(compare_previous_records(
            "Steam",
            &previous_steam.records,
            current,
        ));
        let merged = merge_persisted_records(previous_steam.records, current.clone());
        if let Err(error) = write_registry_account_file("SteamAccounts", &merged) {
            report
                .signals
                .push(format!("Audit Steam registry write failed: {error}"));
        }
    }

    if let Some(current) = snapshot.minecraft.as_ref() {
        report.signals.extend(compare_previous_records(
            "Minecraft",
            &previous_minecraft.records,
            current,
        ));
        let merged = merge_persisted_records(previous_minecraft.records, current.clone());
        if let Err(error) = write_minecraft_account_file(&merged) {
            report
                .signals
                .push(format!("Audit Minecraft file write failed: {error}"));
        }
    }

    if let Err(error) = write_audit_manifest() {
        report
            .signals
            .push(format!("Audit manifest write failed: {error}"));
    }

    report.registry_path = registry_audit_path();
    report.minecraft_file_path = Some(minecraft_audit_file().display().to_string());
    report.manifest_file_path = Some(audit_manifest_file().display().to_string());
    report.signals.sort();
    report.signals.dedup();
    let event_log_warnings = emit_event_log_signals(&report.signals);
    report.signals.extend(event_log_warnings);
    report.signals.sort();
    report.signals.dedup();
    report
}

fn inspect_audit_integrity() -> AuditReport {
    let mut report = AuditReport {
        registry_path: registry_audit_path(),
        minecraft_file_path: Some(minecraft_audit_file().display().to_string()),
        manifest_file_path: Some(audit_manifest_file().display().to_string()),
        ..AuditReport::default()
    };

    let manifest = read_audit_manifest();
    let discord_records = read_registry_account_file("DiscordAccounts");
    let steam_records = read_registry_account_file("SteamAccounts");
    let minecraft_records = read_minecraft_account_file();
    let registry_exists = registry_audit_exists();
    let minecraft_file_exists = minecraft_audit_file().is_file();

    report
        .signals
        .extend(missing_source_signals("Discord", &discord_records.records));
    report
        .signals
        .extend(missing_source_signals("Steam", &steam_records.records));
    report.signals.extend(missing_source_signals(
        "Minecraft",
        &minecraft_records.records,
    ));

    if let Some(manifest) = manifest {
        if (manifest.registry_had_discord || manifest.registry_had_steam) && !registry_exists {
            report
                .signals
                .push("DETECT: audit registry traces were removed".to_string());
        }
        if manifest.minecraft_had_records && !minecraft_file_exists {
            report
                .signals
                .push("DETECT: Minecraft audit file was removed".to_string());
        }
    } else if registry_exists || minecraft_file_exists {
        report
            .signals
            .push("DETECT: audit manifest is missing while traces still exist".to_string());
    }

    report
}

fn missing_source_signals(label: &str, records: &[PersistedAccountRecord]) -> Vec<String> {
    let mut out = Vec::new();
    ...
    for record in records {
        for (index, source) in record.sources.iter().enumerate() {
            ...
            let kind = record
                ...
                .get(index)
                .map(String::as_str)
                .unwrap_or_else(|| classify_audit_source(source));
            if kind != "stable_file" {
                continue;
            }
            ...
            out.push(format!(
                "DETECT: {label} audit source missing for {} ({}) -> {}",
                record.label, record.id, source
            ));
        }
    }
...
fn classify_audit_source(source: &str) -> &'static str {
    let lower = source.replace('/', "\\").to_ascii_lowercase();
    if lower.contains("\\local storage\\leveldb\\")
        || lower.ends_with(".ldb")
        || lower.ends_with(".log")
    {
        return "volatile_leveldb";
    }
    if lower.contains("\\user data\\")
        || lower.contains("\\mozilla\\firefox\\profiles\\")
        || lower.ends_with("history")
        || lower.ends_with("cookies")
        || lower.ends_with("cookies.sqlite")
        || lower.ends_with("places.sqlite")
    {
        return "browser_artifact";
    }
    if lower.contains("\\steam\\userdata\\") {
        return "steam_userdata";
    }
    if lower.starts_with("hkcu\\") || lower.starts_with("hklm\\") {
        return "registry";
    }
    "stable_file"
}

fn emit_event_log_signals(signals: &[String]) -> Vec<String> {
    let mut warnings = Vec::new();
    for signal in signals {
        if signal.contains("DETECT:") {
            if let Err(error) = write_windows_event(signal) {
                warnings.push(format!("Windows Event Log write failed: {error}"));
            }
        }
    }
    warnings
...
#[cfg(windows)]
fn write_windows_event(message: &str) -> Result<(), String> {
    let trimmed = message.chars().take(900).collect::<String>();
    ...
    .map_err(|error| format!("eventcreate.exe unavailable: {error}"))?;
    if output.status.success() {
        return Ok(());
    }
    let stderr = String::from_utf8_lossy(&output.stderr).trim().to_string();
    if stderr.is_empty() {
        Err(format!("eventcreate.exe exited with {}", output.status))
    } else {
        Err(stderr)
    }
}
...
#[cfg(not(windows))]
fn write_windows_event(_message: &str) -> Result<(), String> {
    Ok(())
}

fn compare_previous_records(
    label: &str,
    previous: &[PersistedAccountRecord],
    current: &[PersistedAccountRecord],
) -> Vec<String> {
    ...
    previous
        .iter()
        .filter(|record| !current_ids.contains(&record.id.to_ascii_lowercase()))
        .map(|record| {
            format!(
                "DETECT: {label} account disappeared from current max scan but exists in audit: {} ({})",
...
fn merge_persisted_records(
    previous: Vec<PersistedAccountRecord>,
    current: Vec<PersistedAccountRecord>,
) -> Vec<PersistedAccountRecord> {
    let mut map = HashMap::<String, PersistedAccountRecord>::new();
    for record in previous {
        map.insert(record.id.to_ascii_lowercase(), record);
    }
    for record in current {
        ...
        map.entry(key)
            .and_modify(|existing| {
                ...
                existing.last_seen = record.last_seen.clone();
                existing.sources = merge_sources(&existing.sources, &record.sources);
                existing.source_kinds = existing
                    .sources
                    .iter()
                    .map(|source| classify_audit_source(source).to_string())
                    .collect();
...
fn merge_sources(left: &[String], right: &[String]) -> Vec<String> {
    let mut sources = left.iter().chain(right.iter()).cloned().collect::<Vec<_>>();
    sources.sort();
    sources.dedup();
    sources
}

fn audit_data_dir() -> PathBuf {
    dirs::data_local_dir()
...
fn minecraft_audit_file() -> PathBuf {
    audit_data_dir().join("minecraft_accounts.json")
}

fn audit_manifest_file() -> PathBuf {
    audit_data_dir().join("audit_manifest.json")
}

fn read_minecraft_account_file() -> PersistedAccountFile {
    let path = minecraft_audit_file();
    let Ok(text) = fs::read_to_string(path) else {
        return PersistedAccountFile::default();
    };
    serde_json::from_str(&text).unwrap_or_default()
}

fn write_minecraft_account_file(records: &[PersistedAccountRecord]) -> Result<(), String> {
    let path = minecraft_audit_file();
    if let Some(parent) = path.parent() {
        fs::create_dir_all(parent)
            .map_err(|error| format!("create {} failed: {error}", parent.display()))?;
    }
    let file = PersistedAccountFile {
        version: AUDIT_VERSION,
        updated_at: Local::now().to_rfc3339(),
        records: records.to_vec(),
    };
    let json = serde_json::to_vec_pretty(&file)
        .map_err(|error| format!("serialize minecraft audit failed: {error}"))?;
    fs::write(&path, json).map_err(|error| format!("write {} failed: {error}", path.display()))
}

fn read_audit_manifest() -> Option<AuditManifest> {
    let text = fs::read_to_string(audit_manifest_file()).ok()?;
    serde_json::from_str(&text).ok()
}

fn write_audit_manifest() -> Result<(), String> {
    let path = audit_manifest_file();
    if let Some(parent) = path.parent() {
        fs::create_dir_all(parent)
            .map_err(|error| format!("create {} failed: {error}", parent.display()))?;
    }
    let discord = read_registry_account_file("DiscordAccounts");
    let steam = read_registry_account_file("SteamAccounts");
    let minecraft = read_minecraft_account_file();
    let manifest = AuditManifest {
        version: AUDIT_VERSION,
        updated_at: Local::now().to_rfc3339(),
        registry_had_discord: !discord.records.is_empty(),
        registry_had_steam: !steam.records.is_empty(),
        minecraft_had_records: !minecraft.records.is_empty(),
    };
    let json = serde_json::to_vec_pretty(&manifest)
        .map_err(|error| format!("serialize audit manifest failed: {error}"))?;
    fs::write(&path, json).map_err(|error| format!("write {} failed: {error}", path.display()))
}

#[cfg(windows)]
fn registry_audit_path() -> Option<String> {
    Some(format!(r"HKCU\{AUDIT_REGISTRY_PATH}"))
}
...
#[cfg(windows)]
fn registry_audit_exists() -> bool {
    use winreg::enums::{HKEY_CURRENT_USER, KEY_READ};
    use winreg::RegKey;
    RegKey::predef(HKEY_CURRENT_USER)
        .open_subkey_with_flags(AUDIT_REGISTRY_PATH, KEY_READ)
        .is_ok()
}
...
#[cfg(windows)]
fn read_registry_account_file(value_name: &str) -> PersistedAccountFile {
    use winreg::enums::{HKEY_CURRENT_USER, KEY_READ};
    use winreg::RegKey;
    let root = RegKey::predef(HKEY_CURRENT_USER);
    let Ok(key) = root.open_subkey_with_flags(AUDIT_REGISTRY_PATH, KEY_READ) else {
        return PersistedAccountFile::default();
    };
    let Ok(text) = key.get_value::<String, _>(value_name) else {
        return PersistedAccountFile::default();
    };
    serde_json::from_str(&text).unwrap_or_default()
}
...
#[cfg(not(windows))]
fn read_registry_account_file(_value_name: &str) -> PersistedAccountFile {
    PersistedAccountFile::default()
}

#[cfg(windows)]
fn write_registry_account_file(
    value_name: &str,
    records: &[PersistedAccountRecord],
) -> Result<(), String> {
    use winreg::enums::HKEY_CURRENT_USER;
    use winreg::RegKey;
    let root = RegKey::predef(HKEY_CURRENT_USER);
    let (key, _) = root
        .create_subkey(AUDIT_REGISTRY_PATH)
        .map_err(|error| format!("open HKCU\\{AUDIT_REGISTRY_PATH} failed: {error}"))?;
    let file = PersistedAccountFile {
        version: AUDIT_VERSION,
        updated_at: Local::now().to_rfc3339(),
        records: records.to_vec(),
    ...
    let json = serde_json::to_string(&file)
        .map_err(|error| format!("serialize registry audit failed: {error}"))?;
    key.set_value(value_name, &json)
        .map_err(|error| format!("write registry {value_name} failed: {error}"))
}

#[cfg(not(windows))]
fn write_registry_account_file(
    _value_name: &str,
    _records: &[PersistedAccountRecord],
) -> Result<(), String> {
    Ok(())
}

#[cfg(windows)]
fn read_machine_guid(warnings: &mut Vec<String>) -> Option<String> {
    use winreg::enums::{HKEY_LOCAL_MACHINE, KEY_READ};
    use winreg::RegKey;
    let root = RegKey::predef(HKEY_LOCAL_MACHINE);
    match root.open_subkey_with_flags(r"SOFTWARE\Microsoft\Cryptography", KEY_READ) {
        Ok(key) => match key.get_value::<String, _>("MachineGuid") {
            Ok(value) => Some(value),
            Err(error) => {
                warnings.push(format!("MachineGuid unavailable: {error}"));
                None
            }
        },
        Err(error) => {
            warnings.push(format!("Cryptography registry unavailable: {error}"));
            None
        }
    }
}
...
#[cfg(not(windows))]
fn read_machine_guid(_warnings: &mut Vec<String>) -> Option<String> {
    None
}
...
mod tests {
    ...
    fn minecraft_placeholder_requires_multiple_logs() {
        ...
        let (kept, _, _, _) = classify_minecraft_candidate(&candidate);
        assert!(!kept);

        candidate.log_hits = 3;
        let (kept, confidence, _, _) = classify_minecraft_candidate(&candidate);
        assert!(kept);
    ...
    #[test]
    fn steam_loginusers_parser_extracts_basic_account() {
        let sample = r#"
"users"
{
    "76561198000000000"
...
"#;
        ...
        let users = parse_loginusers_vdf(sample);
        assert_eq!(users.len(), 1);
        assert_eq!(users[0].steam_id64, "76561198000000000");
        assert_eq!(users[0].account_name.as_deref(), Some("alpha"));
        assert_eq!(users[0].persona_name.as_deref(), Some("Alpha"));
    }

    #[test]
    fn audit_merge_preserves_old_records_and_adds_new_sources() {
        let previous = vec![persisted_record(
            "old",
            "Old Account",
            &["old-source".to_string()],
        )];
        let current = vec![
            persisted_record("old", "Old Account Renamed", &["new-source".to_string()]),
            persisted_record("fresh", "Fresh Account", &["fresh-source".to_string()]),
        ];

        let merged = merge_persisted_records(previous, current);
        let old = merged
            .iter()
            .find(|record| record.id == "old")
            .expect("old record must stay in audit storage");
        assert_eq!(old.label, "Old Account Renamed");
        assert!(old.sources.contains(&"old-source".to_string()));
        assert!(old.sources.contains(&"new-source".to_string()));
        assert!(merged.iter().any(|record| record.id == "fresh"));
    }
}
