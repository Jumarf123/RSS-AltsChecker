...
impl ConfidenceLevel {
    ...
    pub fn description(self) -> &'static str {
        match self {
            Self::Max => "Высокая уверенность: подтверждено надежными признаками",
            Self::Low => "Возможное совпадение: нужна ручная проверка",
        }
    }
}
...
pub struct SteamBanStatus {
    ...
    pub economy_ban: Option<String>,
    pub faceit_status: Option<String>,
...
pub fn collect_system_hwid() -> SystemHwid {
    ...
    let primary_hwid = machine_guid
        .clone()
        .or_else(|| {
            computer_name
                .as_ref()
                .zip(user_name.as_ref())
                .map(|(computer, user)| format!("{computer}-{user}"))
        })
        .unwrap_or_else(|| "unknown".to_string());
...
fn scan_steam_accounts(
    ...
    let mut out = map
        ...
        .map(|entry| {
            ...
            SteamAlt {
                profile_url: format!("https://steamcommunity.com/profiles/{}", entry.steam_id64),
...
fn enrich_steam_accounts(accounts: &mut [SteamAlt], warnings: &mut Vec<String>) {
    ...
    enrich_faceit_status(accounts, warnings);
}
...
fn enrich_steam_profiles_from_xml(accounts: &mut [SteamAlt], warnings: &mut Vec<String>) {
    ...
    for account in accounts.iter_mut() {
        ...
        if account.avatar_url.is_none() {
            account.avatar_url = extract_xml_tag(&text, "avatarMedium")
                .or_else(|| extract_xml_tag(&text, "avatarFull"));
        }
    }
}

fn enrich_steam_bans(accounts: &mut [SteamAlt], warnings: &mut Vec<String>) {
    let Ok(api_key) = env::var("STEAM_WEB_API_KEY") else {
        for account in accounts {
            account
                .bans
                .notes
                .push("Steam bans: нужен STEAM_WEB_API_KEY".to_string());
        }
        return;
    };
    ...
    for account in accounts {
        ...
        account.bans.days_since_last_ban = player
            ...
            .and_then(Value::as_u64)
            .map(|v| v as u32);
        account.bans.economy_ban = player
            .get("EconomyBan")
            .and_then(Value::as_str)
            .map(ToOwned::to_owned);
    }
...
fn enrich_faceit_status(accounts: &mut [SteamAlt], warnings: &mut Vec<String>) {
    let Ok(api_key) = env::var("FACEIT_API_KEY") else {
        for account in accounts {
            account.bans.faceit_status = Some("не проверено: нужен FACEIT_API_KEY".to_string());
        }
        return;
    };
    if api_key.trim().is_empty() {
    ...
    for account in accounts {
        ...
        let player_response = match agent
            .get(&player_url)
            ...
            .call()
        {
            Ok(response) => response,
            Err(_) => {
                account.bans.faceit_status =
                    Some("профиль не найден или API недоступен".to_string());
                continue;
            }
        };
        let player = match player_response.into_json::<Value>() {
            Ok(value) => value,
            Err(error) => {
                warnings.push(format!(
                    "FACEIT profile API response parse failed for {}: {error}",
                    account.steam_id64
                ));
                account.bans.faceit_status = Some("ответ API не прочитан".to_string());
                continue;
            }
        };

        let Some(player_id) = player.get("player_id").and_then(Value::as_str) else {
            account.bans.faceit_status = Some("профиль не найден".to_string());
            continue;
        };
        let bans_url = format!("https://open.faceit.com/data/v4/players/{player_id}/bans");
        let bans_response = match agent
            .get(&bans_url)
            .set("Authorization", &format!("Bearer {api_key}"))
            .call()
        {
            ...
            Err(error) => {
                warnings.push(format!(
                    "FACEIT bans API unavailable for {}: {error}",
                    account.steam_id64
                ));
                account.bans.faceit_status = Some("API банов недоступен".to_string());
                continue;
            }
        };
        match bans_response.into_json::<Value>() {
            Ok(value) => {
                let count = value
                    .get("items")
                    .and_then(Value::as_array)
                    .map(Vec::len)
                    .unwrap_or(0);
                account.bans.faceit_status = Some(if count == 0 {
                    "банов не найдено".to_string()
                } else {
                    format!("найдено банов: {count}")
                });
            }
            Err(error) => {
                warnings.push(format!(
                    "FACEIT bans API response parse failed for {}: {error}",
                    account.steam_id64
                ));
                account.bans.faceit_status = Some("ответ API банов не прочитан".to_string());
            }
        }
    }
}
